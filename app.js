...
const S={
  ws:null,candleWs:null,symbol:null,marketName:'',prices:[],times:[],candles:[],timeframe:60,engineCandles:[],engineTimeframe:60,engineHistoryReady:false,
  ...
  algorithm:null,historical:null,strategies:[],algoTick:0,algoAt:0,
...
function handleMainMessage(d){
  ...
  if(d.msg_type==='candles'&&Array.isArray(d.candles)&&d.req_id===S.engineHistoryReq){
    S.engineCandles=normalizeCandles(d.candles);S.engineHistoryReady=S.engineCandles.length>=24;
    runEngine(S.prices.at(-1),S.times.at(-1)||nowSec());
  }
  if(d.msg_type==='tick'){
    ...
    updateEngineCandle(q,ep);runEngine(q,ep);
  }
}
function subscribeMarket(){
  ...
  S.engineHistoryReq=++S.req;S.engineCandles=[];S.engineHistoryReady=false;send({ticks_history:S.symbol,count:1000,end:'latest',style:'candles',granularity:S.engineTimeframe,req_id:S.engineHistoryReq});
...
function updateEngineCandle(price,epoch){
  const tf=S.engineTimeframe,bucket=Math.floor(epoch/tf)*tf;let c=S.engineCandles.at(-1);
  if(!c||bucket<c.time)return;
  if(bucket>c.time){c={time:bucket,open:c.close,high:Math.max(c.close,price),low:Math.min(c.close,price),close:price};S.engineCandles.push(c);if(S.engineCandles.length>500)S.engineCandles.shift()}
...
function requestCandles(){
  ...
  const symbol=S.symbol,tf=S.timeframe,baseTf=tf===172800?86400:tf,req=++S.req;S.historyReq=req;S.candles=[];S.historyStatus='CARGANDO OHLC';updateBadge();
...
function addMarker(type,dir,price,time,text,persistent=true){
  ...
  const cleanText='';S.markers.push({id,type,dir,price,time:t,eventTime:t,text:cleanText,persistent});if(S.markers.length>60)S.markers.splice(0,S.markers.length-60);renderMarkers();
}
...
function metrics(){
  const cs=S.engineCandles.slice(-60);if(cs.length<24)return null;const closes=cs.map(c=>c.close),ranges=cs.map(c=>c.high-c.low),A=atr(cs,14)||sd(closes.slice(-20))||Math.abs(closes.at(-1))*.00001;
...
function historicalAnalog(){
  const cs=S.engineCandles.slice();const n=12,forward=6;
  ...
  for(let end=n;end<=maxEnd;end++){
    const window=cs.slice(end-n,end), sig=patternSignature(window,n);const sim=patternSimilarity(current,sig);
    if(sim<.68)continue;
...
function tfContext(){
  ...
  for(const tf of [300,900,1800,3600]){
    const cs=aggregateCandles(S.engineCandles,tf).slice(-24);if(cs.length<8)continue;
...
function evaluateStrategies(m,sp,hist){
  const cs=m?.cs||[], last=m?.last;if(!m||!last)return [];
...
function algorithmicEngine(m,sp){
  const hist=historicalAnalog();const strategies=evaluateStrategies(m,sp,hist);const top=strategies[0]||{name:'Sin estrategia',dir:'NEUTRAL',score:0};
...
function createSetup(m,price,epoch){
  ...
  S.setup={id:'S'+Date.now(),dir:m.dir,createdPrice:price,createdEpoch:epoch,createdCandle:m.last.time,trigger,invalid,atr:m.A,conf:m.conf,expiresAt:epoch+S.engineTimeframe*3,score:m.best};
...
function cancelSetup(reason,late=false,price=null){
  ...
  S.setup=null;S.cooldownUntil=(S.times.at(-1)||nowSec())+Math.max(15,S.engineTimeframe*.5);
}
...
function closeSignal(reason,price,epoch){
  ...
  logEvent('SALIDA '+reason,a.dir,price,a.conf,'CICLO CERRADO');queueSpeech(`Salir ahora. ${reason}`,90);S.activeSignal=null;S.cooldownUntil=epoch+S.engineTimeframe;
}
...
function runEngine(price,epoch){
  ...
  if(!S.algorithm||S.prices.length-S.algoTick>=5||Date.now()-S.algoAt>1200){S.algorithm=algorithmicEngine(m,sp);S.historical=S.algorithm.historical;S.strategies=S.algorithm.strategies;S.algoTick=S.prices.length;S.algoAt=Date.now();updateAlgorithmUI(S.algorithm)}
  ...
  if(normalCandidate||spikeCandidate){
    ...
    const bucket=m.last.time;if(S.lastSetupEvalBucket!==bucket){S.lastSetupEvalBucket=bucket;createSetup(m,price,epoch);if(S.signal)S.signal.reason+=` · Algoritmo ${algo.score}% · ${algo.strategy}`;updateSignalUI();return}
  }
