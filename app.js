...
/* ==========================================================
   V1.6.0 · MOTOR DE SEÑALES MR
   DEMO / MANUAL primero. AUTO = genera señales, NO envía trades.
   ========================================================== */
S.demoPosition=null; S.signalLines=[]; S.signalState='WAIT'; S.lastSignalVoice='';
...
function drawSignalLevels(s){clearSignalLines();if(!s)return;addSignalLine(s.alert,'POSIBLE', '#d9a900',2);addSignalLine(s.entry,'ENTRADA','#0bad63',0);addSignalLine(s.exit,'OBJETIVO','#e24a59',2);addSignalLine(s.stop,'STOP','#f08a35',2)}
function signalUiV160(s){
 setText('stopLevel',fmtPrice(s.stop));setText('holdTime',s.hold||'—');setText('graphAlert',fmtPrice(s.alert));setText('graphEntry',fmtPrice(s.entry));setText('graphStop',fmtPrice(s.stop));setText('graphHold',s.hold||'—');setText('graphExecMode',($('executionMode')?.value||'manual').toUpperCase());
 const o=$('signalOverlay');if(o){o.className='signal-overlay '+(s.phase.includes('ENTRAR')?'entry':s.phase.includes('POSIBLE')?'alert':s.phase.includes('NO OPERAR')?'wait':'');o.textContent=s.phase+(Number.isFinite(s.alert)?` · ${fmtPrice(s.alert)}`:'')}
 drawSignalLevels(s);
}
function analyze(){
 const p=S.prices,n=p.length;els.ticks.textContent=n;if(n<50){syncUI();return}
 const cur=p[n-1],r=p.slice(-80),fast=ema(r.slice(-24),9),slow=ema(r.slice(-55),21),mom=cur-p[n-13],vol=sd(r.slice(-35));
 const atr=atrFromCandles(14)||vol||Math.abs(cur)*.00001||1, unit=Math.max(vol,atr*.35,Math.abs(cur)*.000001);
 const trend=(fast-slow)/unit, momentum=mom/unit;
 ...
 const spikeRisk=Math.min(96,Math.round(18+Math.max(0,largest/unit-1.4)*12+Math.min(30,Math.abs(momentum)*5)));
 const rawScore=trend*.62+momentum*.38; let dir=rawScore>.32?'ALZA ↑':rawScore<-.32?'BAJA ↓':'NEUTRAL';
 let conf=Math.max(48,Math.min(92,Math.round(54+Math.abs(rawScore)*14)));
 let phase='⚠️ NO OPERAR',reason='Esperando una estructura con mayor confirmación.',alert=null,entry=null,target=null,stop=null,hold='—';
 if(dir!=='NEUTRAL'){
   const up=dir.startsWith('ALZA');
   const projected=up?Math.max(support,cur-atr*.55):Math.min(resistance,cur+atr*.55);
   alert=projected; target=projected+(up?1:-1)*atr*1.8; stop=projected-(up?1:-1)*atr*.9; hold=`${Math.max(1,Math.min(4,Math.round(2.6-Math.abs(rawScore)*.35)))}–${Math.max(2,Math.min(6,Math.round(4.5-Math.abs(rawScore)*.25)))} velas`;
   if(conf>=62){phase='🟡 POSIBLE ENTRADA';reason=`Vigilar ${fmtPrice(projected)}. Tendencia y momentum apuntan ${up?'al alza':'a la baja'}, aún requiere confirmación.`}
   const near=Math.abs(cur-projected)<=atr*.7;
   const aligned=(up&&fast>slow&&mom>0)||(!up&&fast<slow&&mom<0);
   if(conf>=72&&near&&aligned&&spikeRisk<72){phase='🟢 ENTRAR AHORA';entry=cur;target=cur+(up?1:-1)*atr*1.8;stop=cur-(up?1:-1)*atr*.9;reason=`Precio en zona y confirmación activa. Vigilar durante ${hold}; salir antes si se invalida.`}
   if(spikeRisk>=78){phase='⚠️ NO OPERAR';entry=null;reason='Riesgo de spike elevado: señal bloqueada hasta nueva confirmación.'}
 }
 const prev=S.signal;S.signal={dir,conf,phase,alert,entry,exit:target,stop,hold,spikeRisk,price:cur,time:Date.now()};
 els.price.textContent=fmtPrice(cur);els.direction.textContent=dir;els.confidence.textContent=conf+'%';els.phase.textContent=phase;els.reason.textContent=reason;els.alert.textContent=fmtPrice(alert);els.entry.textContent=fmtPrice(entry);els.exit.textContent=fmtPrice(target);els.spike.textContent=spikeRisk+'%';els.ind.innerHTML=`Momentum: ${mom.toFixed(5)}<br>Volatilidad: ${vol.toFixed(5)}<br>EMA rápida/lenta: ${fast.toFixed(5)} / ${slow.toFixed(5)}<br>ATR aprox.: ${atr.toFixed(5)}`;
 syncUI();signalUiV160(S.signal);
 const key=phase+'|'+dir+'|'+(alert?fmtPrice(alert):'');
 if((phase.includes('POSIBLE')||phase.includes('ENTRAR')||phase.includes('NO OPERAR'))&&key!==S.lastLogged){S.lastLogged=key;logSignal();if(phase.includes('ENTRAR')||phase.includes('POSIBLE')){const vk=phase+'|'+dir;if(vk!==S.lastSignalVoice){S.lastSignalVoice=vk;speak(phase.includes('ENTRAR')?`Entrar ahora, ${dir.startsWith('ALZA')?'alza':'baja'}`:`Posible entrada ${dir.startsWith('ALZA')?'al alza':'a la baja'}, vigilar nivel ${fmtPrice(alert)}`)}}}
}
function openDemoPosition(){const s=S.signal;if(!s?.phase?.includes('ENTRAR')){speak('Entrada demo no disponible. Espere confirmación del radar.');return}S.demoPosition={id:'D'+Date.now().toString().slice(-6),symbol:S.symbol,dir:s.dir,entry:S.prices.at(-1),target:s.exit,stop:s.stop,opened:Date.now()};renderDemoPosition();speak('Entrada demo registrada')}
function closeDemoPosition(){if(!S.demoPosition)return;const cur=S.prices.at(-1),d=S.demoPosition;const pnl=(cur-d.entry)*(d.dir.startsWith('ALZA')?1:-1);d.closed=Date.now();d.close=cur;d.pnl=pnl;S.demoPosition=null;renderDemoPosition();speak(`Salida demo registrada`)}
function renderDemoPosition(){const box=document.querySelector('.positions');if(!box)return;const d=S.demoPosition;if(!d){box.innerHTML='<span>ID</span><span>MERCADO</span><span>TIPO</span><span>ENTRADA</span><span>PRECIO ACTUAL</span><span>OBJETIVO</span><span>STOP LOSS</span><span>PROFIT</span><span>ESTADO</span>';document.querySelector('.bottom-tabs button.active').textContent='▣ POSICIONES (0)';return}const cur=S.prices.at(-1),pnl=(cur-d.entry)*(d.dir.startsWith('ALZA')?1:-1);box.innerHTML=`<span>${d.id}</span><span>${S.symbol}</span><span>${d.dir}</span><span>${fmtPrice(d.entry)}</span><span>${fmtPrice(cur)}</span><span>${fmtPrice(d.target)}</span><span>${fmtPrice(d.stop)}</span><span>${pnl.toFixed(5)}</span><span class="active-demo">DEMO ACTIVA</span>`;document.querySelector('.bottom-tabs button.active').textContent='▣ POSICIONES (1)'}
$('manualEnter')?.addEventListener('click',openDemoPosition);$('manualExit')?.addEventListener('click',closeDemoPosition);$('executionMode')?.addEventListener('change',()=>{setText('graphExecMode',$('executionMode').value.toUpperCase());if($('executionMode').value==='auto')speak('Modo automático de señales. La ejecución automática permanece bloqueada en esta versión.')});
