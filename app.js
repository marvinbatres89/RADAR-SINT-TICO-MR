'use strict';
/* RADAR SINTÉTICO MR V1.8.1 · GRÁFICO DE SEÑALES + CONFLUENCIA
...
const CFG={confluence:true,spikeGuard:true,historySpikes:true,followDemo:true};
...
function renderMarkers(){
  ...
  const own=S.markers.filter(m=>!m.symbol||m.symbol===S.symbol).sort((a,b)=>(a.eventTime||a.time)-(b.eventTime||b.time)),cut=own.length-RECENT_MARKER_TEXT;
  const arr=own.map((m,i)=>markerSpec(m,i>=cut)).sort((a,b)=>a.time-b.time);
...
function monitorSignal(M,price,epoch,m){
  ...
  if(CFG.spikeGuard&&counterSpike(M,a.dir)&&sp.prep>=SPIKE_EXIT){
  ...
  if(!a.reboundWarned&&favorable>a.atr*.35&&(opposite||extreme)){
    a.reboundWarned=true;const reboundDir=up?'BAJA ↓':'ALZA ↑';addMarker('rebound',reboundDir,price,epoch,'REBOTE',true,M.sym);
  ...
  if(!a.reboundWarned){
    ...
    else setSignal(M,{...base,phase:follow?'💼 EN OPERACIÓN':'🔵 MANTENER',reason:follow?'RADAR sigue su operación: avisa rebote, pérdida de impulso y riesgo de spike.':'La estructura de la entrada sigue vigente.',timing:'SEGUIMIENTO'});
  }
}
...
function updateSignalUI(){
  ...
  const ts=M?.sp?.ticksSince,spk=`${s.spikeRisk||0}%${ts==null?'':` · ⚡${ts}`}`;
...
/* Panel sobre el gráfico: estado, plan, proyección y votos de los motores del mercado en vista. */
function renderHud(){
  ...
  if(!M.historyReady||!M.engineHistoryReady){put('wait',`⏳ ${M.short} · CARGANDO`,'Descargando el histórico del mercado…','','');return}
  ...
  let cls='wait',l1=ph,l2=s.reason||'';
  if(d){const up=d.dir.startsWith('ALZA'),pnl=(d.lastPrice-d.entry)*(up?1:-1),money=pnl*(d.lot||.2)*CONTRACT_SIZE;cls=ph.includes('RIESGO')?'risk':'active';l1=`${ph} · ${up?'BUY ↑':'SELL ↓'}`;l2=`Entrada ${fmt(d.entry)} · P/L ${fmt2(pnl)} pts ≈ ${money<0?'−':''}$${Math.abs(money).toFixed(2)}`}
  else if(a){cls=ph.includes('ENTRA')?'entry':'active';l1=`${ph} ${arrowOf(a.dir)}`;l2=`Entrada ${fmt(a.entry)} · Objetivo ${fmt(a.target)} · Stop ${fmt(a.stop)}`}
  else if(st){const up=st.dir.startsWith('ALZA'),dist=Number.isFinite(price)?(st.trigger-price)*(up?1:-1)/st.atr:NaN;cls=ph.includes('POSIBLE ENTRADA')?'near':'prealert';l1=`${ph} ${arrowOf(st.dir)} · disparador ${fmt(st.trigger)}`;l2=Number.isFinite(dist)?`Falta ${Math.max(0,dist).toFixed(2)} ATR para confirmar · no entrar antes`:'Esperando confirmación'}
  else if(ph.includes('BLOQUEADA'))cls='blocked';else if(ph.includes('EJECUTADO'))cls='late';else if(ph.includes('SALIR'))cls='active';
  let l3='Proyección: sin patrones parecidos todavía';
  if(pj){const mv=pj.mean[pj.mean.length-1],sim=Math.round(pj.sim*100);l3=`Proyección ${PROJ_MINUTES} min: ${Math.abs(mv)<.18?'lateral':`${mv>0?'↑':'↓'} ${fmt2(mv*pj.atr)} pts`} (${pj.count} patrones · parecido ${sim}%${sim<50?' · débil':''})`}
  const pos=d||a;if(pos&&counterSpike(M,pos.dir)&&M.spikeSize)l3+=` · si hay spike ≈ −${M.spikeSize.toFixed(2)} pts`;
  const l4=`${c.favor}/${c.total} estrategias ${arrowOf(dir)}${c.contra?` (${c.contra} en contra)`:''} · Algoritmo ${c.algoScore}% ${arrowOf(c.algoDir)} · Spike ${sp?.prep??0}%${sp?.ticksSince==null?'':` ⚡${sp.ticksSince}`}`;
  put(cls,l1,l2,l3,l4);
}
