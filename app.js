'use strict';
/* RADAR SINTÉTICO MR V1.10.2 · MODOS + VARIAS OPERACIONES + MONTOS + MODO VENTANA
   V1.10.2: MODO VENTANA — en una ventana flotante pequeña (encima de MetaTrader) solo se ven las entradas, los spikes
   y las operaciones abiertas, en letra grande.
   V1.10.1: se actualiza sola (sin copias viejas guardadas en el teléfono) y muestra la versión en pantalla.
...
const S={
  ...
  manualDirection:null,demos:new Map(),hunt:'both',winMode:'auto',voice:true,voiceQueue:[],voiceSpeaking:false,bootReadyAt:0,
...
function saveNow(){
  ...
  try{
    ...
    localStorage.setItem(STORE_KEY,JSON.stringify({v:1,at:Date.now(),power:S.power,symbol:S.symbol,tf:S.timeframe,voice:S.voice,lot:$('lotSize')?.value,mode:$('executionMode')?.value,spikeOn:S.spikeOn,hunt:S.hunt,bg:S.bgMode,hud:!!S.hudCompact,win:S.winMode,demos:Object.fromEntries(S.demos),markets:mk,markers,log:S.log.slice(0,HISTORY_ROWS),stats:S.stats}));
  }catch{}
}
function restoreState(){
  ...
  S.spikeOn=st.spikeOn!==false;S.hunt=['normal','spike','both'].includes(st.hunt)?st.hunt:(S.spikeOn?'both':'normal');S.bgMode=!!st.bg;S.hudCompact=!!st.hud;S.winMode=['auto','on','off'].includes(st.win)?st.win:'auto';
...
/* ACTUALIZACIÓN: el service worker pide siempre la versión nueva a GitHub (sin copias viejas); si llega una
   versión nueva mientras la app está abierta, se guarda todo y se recarga una sola vez. */
if('serviceWorker' in navigator){
  const hadCtrl=!!navigator.serviceWorker.controller;let reloaded=false;
  navigator.serviceWorker.addEventListener?.('controllerchange',()=>{if(!hadCtrl||reloaded)return;reloaded=true;try{saveNow()}catch{}location.reload()});
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js',{updateViaCache:'none'}).then(r=>r.update?.()).catch(()=>{}));
...
function updateBadge(){const tf=document.querySelector('.tf.active')?.textContent||'1m';setText('chartBadge',`${tf} · ${S.historyStatus==='CARGANDO OHLC'?'cargando…':S.candles.length+' velas'} · motor M1 · v${APP_VERSION}`)}
...
function renderScanner(){
  ...
  renderMini(rows,live);
}

/* ---------- MODO VENTANA (ventana flotante pequeña, por ejemplo encima de MetaTrader 5) ----------
   Automático cuando la ventana es chica (≤ 430 × 560 px o ≤ 300 px de ancho). Solo muestra lo que se puede operar:
   entradas, "cerca", pre-alertas, spikes y las operaciones abiertas, en letra grande. */
function winSmall(){const w=window.innerWidth||0,h=window.innerHeight||0;return w>0&&((w<=430&&h<=560)||w<=300)}
function applyWinMode(){const on=S.winMode==='on'||(S.winMode==='auto'&&winSmall());const was=document.documentElement.classList.contains('mini-on');document.documentElement.classList.toggle('mini-on',on);
  setText('winToggle',`🪟 Ventana: ${S.winMode==='on'?'SIEMPRE':S.winMode==='off'?'NUNCA':'AUTO'}`);if(on!==was)scheduleScan(true)}
function miniExtra(M,t){
  const d=S.demos.get(M.sym),a=M.activeSignal,k=M.spk?.trade,st=M.setup,price=M.prices.at(-1),lot=d?.lot||lotNow();
  if(d)return {txt:money(demoPnl(d)*lot*CONTRACT_SIZE),sub:`TP ${gain$(d.target-d.entry,lot)} · SL ${loss$(d.entry-d.stop,lot)}`,cls:demoPnl(d)>=0?'up':'down'};
  if(k){const left=Math.max(0,k.H-((M.times.at(-1)||k.entryEpoch)-k.entryEpoch));return {txt:`⏱ ${clock(left*tickSec(M))}`,sub:`${k.dir.startsWith('ALZA')?'BUY':'SELL'} · TP ${gain$(k.tp-k.entry)} · SL ${loss$(k.entry-k.stop)}`}}
  if(a)return {txt:a.dir.startsWith('ALZA')?'BUY':'SELL',sub:`TP ${gain$(a.target-a.entry)} · SL ${loss$(a.entry-a.stop)}${a.mc?` · objetivo ${pct(a.mc.pTP)}`:''}`};
  if(st){const dist=Number.isFinite(price)?Math.max(0,(st.trigger-price)*(st.dir.startsWith('ALZA')?1:-1)/st.atr):null;return {txt:`${st.conf}%`,sub:`${st.dir.startsWith('ALZA')?'BUY':'SELL'} si cruza ${fmtP(st.trigger)}${dist!=null?` (falta ${dist.toFixed(2)} ATR)`:''}`}}
  if(M.spk?.zone||t.cls==='spkzone'){const p=pSpikeMin(M);return {txt:p==null?'—':`${pct(p)}/min`,sub:M.spk?.zone?.why||spikeAdvantage(M)||''}}
  return {txt:'',sub:''};
}
function renderMini(rows,live){
  const list=$('miniList');if(!list||!document.documentElement.classList.contains('mini-on'))return;
  setText('miniConn',live?'● EN VIVO':'● SIN CONEXIÓN');const c=$('miniConn');if(c)c.className='mini-conn'+(live?' on':'');
  const act=rows.filter(r=>r.t.tier>=2&&r.t.cls!=='exit'||S.demos.has(r.M.sym)).slice(0,8);let html='';
  if(act.length)html=act.map(({M,t,f})=>{const x=miniExtra(M,t);return `<div class="mini-row ${t.cls}" data-sym="${esc(M.sym)}"><b class="mr-label">${esc(t.label)}</b><span class="mr-mk">${esc(M.short)} ${arrowOf(f.dir)}</span><b class="mr-x ${x.cls||''}">${esc(x.txt)}</b>${x.sub?`<small>${esc(x.sub)}</small>`:''}</div>`}).join('');
  else{const mode=S.hunt==='spike'?'⚡ Spikes':S.hunt==='normal'?'📈 Normales':'Ambos';
    html=`<p class="mini-empty">⚪ Sin entradas ahora<br><small>Vigilando ${rows.length} mercados · ${mode}${S.voice?' · la voz avisa':''}</small></p>`;
    if(S.hunt!=='normal'){const top=rows.map(r=>({M:r.M,p:pSpikeMin(r.M)})).filter(x=>x.p!=null).sort((a,b)=>b.p-a.p).slice(0,3);
      if(top.length)html+=`<div class="mini-top"><small>Spike en el próximo minuto (al azar):</small>${top.map(x=>`<span data-sym="${esc(x.M.sym)}">${esc(x.M.short)} ${arrowOf(x.M.bias)} ${pct(x.p)}</span>`).join('')}</div>`}}
  const n=S.demos.size,tot=[...S.demos.values()].reduce((s,d)=>s+demoPnl(d)*(d.lot||.2)*CONTRACT_SIZE,0);
  const foot=`${n?`💼 ${n} abierta${n>1?'s':''} · <b class="${tot>=0?'up':'down'}">${money(tot)}</b>`:'💼 sin operaciones'} · ${$('executionMode')?.value==='auto'?'🤖 AUTO':'✋ MANUAL'}`;
  if(list._last!==html){list.innerHTML=html;list._last=html}const fo=$('miniFoot');if(fo&&fo._last!==foot){fo.innerHTML=foot;fo._last=foot}
  setText('miniVoice',S.voice?'🔊':'🔇');
}
...
$('voice').onclick=()=>{
  S.voice=!S.voice;$('voice').textContent=`🔊 Voz: ${S.voice?'ON':'OFF'}`;setText('miniVoice',S.voice?'🔊':'🔇');markDirty();
...
$('miniVoice')?.addEventListener('click',()=>$('voice').onclick());
$('winToggle')?.addEventListener('click',()=>{S.winMode=S.winMode==='auto'?'on':S.winMode==='on'?'off':'auto';applyWinMode();markDirty()});
$('miniFull')?.addEventListener('click',()=>{S.winMode='off';applyWinMode();markDirty()});
$('miniList')?.addEventListener('click',e=>{const b=e.target?.closest?.('[data-sym]');if(b)setViewMarket(b.dataset.sym)});
window.addEventListener('resize',()=>applyWinMode());
restoreState();renderPower();renderToggles();renderLog();renderStats();renderDemo();renderInstallBanner();applyWinMode();
