...
const S={
  ...
  lastSetupEvalBucket:null,lastReboundMarker:'',lastSpikeEventAt:0
};
...
function addMarker(type,dir,price,time,text,persistent=true){
  ...
  S.markers.push({id,type,dir,price,time:t,text,persistent});if(S.markers.length>60)S.markers.splice(0,S.markers.length-60);renderMarkers();
}
...
function spikeRisk(){
  const p=S.prices.slice(-160);if(p.length<30)return {risk:0,text:'SIN DATOS'};const ds=p.slice(1).map((v,i)=>v-p[i]);const sigma=sd(ds)||1e-9,max=Math.max(...ds.map(Math.abs));const recent=Math.max(...ds.slice(-18).map(Math.abs));const compression=sd(ds.slice(-18))/(sd(ds.slice(-80))||sigma);let risk=18+Math.max(0,recent/sigma-1.4)*9+(compression<.7?14:0);risk=clamp(Math.round(risk),8,92);
  const name=currentMarketLabel().toLowerCase(),dir=name.includes('boom')?'ALZA ↑':name.includes('crash')?'BAJA ↓':'NEUTRAL';return {risk,text:`${dir==='NEUTRAL'?'RIESGO':dir} · ${risk}%`};
}
...
function createSetup(m,price,epoch){
  ...
  logEvent('PRE-ALERTA',m.dir,price,m.conf,'A TIEMPO');queueSpeech(`Pre alerta ${up?'al alza':'a la baja'}. Vigilar ${fmt(trigger)}`,30);
}
...
function confirmSetup(price,epoch,m){
  ...
  logEvent('ENTRADA',S.activeSignal.dir,price,S.activeSignal.conf,'A TIEMPO');queueSpeech(`Entrar ahora, ${up?'alza':'baja'}`,70);
}
...
/* ---------- DEMO MANUAL ---------- */
function chooseDirection(dir){S.manualDirection=dir;$('manualUp').classList.toggle('selected',dir==='ALZA ↑');$('manualDown').classList.toggle('selected',dir==='BAJA ↓');queueSpeech(`Dirección manual ${dir.startsWith('ALZA')?'alza':'baja'} seleccionada`,10)}
function openDemo(){
  if(S.demoPosition){queueSpeech('Ya existe una operación demo activa.',15);return}const price=S.prices.at(-1);if(!Number.isFinite(price))return;let dir=S.manualDirection;if(!dir&&S.activeSignal)dir=S.activeSignal.dir;if(!dir){queueSpeech('Seleccione BUY alza o SELL baja.',15);return}
  ...
  S.demoPosition={id:'D'+Date.now().toString().slice(-6),dir,entry:price,opened:Date.now(),target:match?S.activeSignal.target:price+(up?1:-1)*A*1.6,stop:match?S.activeSignal.stop:price-(up?1:-1)*A*.8,bestPrice:price,source:match?'RADAR':'MANUAL'};
...
function renderDemo(){
  const d=S.demoPosition;if(!d){setText('positionState','SIN POSICIÓN');setText('positionCard','Sin operación activa.');return}const cur=S.prices.at(-1),pnl=(cur-d.entry)*(d.dir.startsWith('ALZA')?1:-1);setText('positionState',`${d.dir.startsWith('ALZA')?'BUY':'SELL'} · ${fmt(d.entry)} · P/L ${pnl>=0?'+':''}${fmt(pnl)}`);$('positionCard').innerHTML=`<b>${d.source} · ${d.dir}</b><br>Entrada: ${fmt(d.entry)}<br>Actual: ${fmt(cur)}<br>Objetivo: ${fmt(d.target)}<br>Stop: ${fmt(d.stop)}<br>P/L: ${pnl>=0?'+':''}${fmt(pnl)}`
}
...
$('manualUp').onclick=()=>chooseDirection('ALZA ↑');$('manualDown').onclick=()=>chooseDirection('BAJA ↓');$('manualEnter').onclick=openDemo;$('manualExit').onclick=()=>closeDemo('SALIDA MANUAL');
